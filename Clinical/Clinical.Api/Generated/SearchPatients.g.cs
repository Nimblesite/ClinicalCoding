...
/// <summary>
/// Extension methods for 'SearchPatients'.
/// </summary>
public static partial class SearchPatientsExtensions
{
    /// <summary>
    /// Executes 'SearchPatients.sql' and maps results.
    /// </summary>
    /// <param name="connection">Open NpgsqlConnection connection.</param>
    /// <param name="term">Query parameter.</param>
    /// <returns>Result of records or SQL error.</returns>
    public static async Task<Result<ImmutableList<SearchPatients>, SqlError>> SearchPatientsAsync(this NpgsqlConnection connection, object term)
    {
        ...
        try
        {
            var results = ImmutableList.CreateBuilder<SearchPatients>();

            using (var command = new NpgsqlCommand(sql, connection))
            {
                if (term is not null and not DBNull)
                    command.Parameters.AddWithValue("@term", term);
                else
                    command.Parameters.Add(new NpgsqlParameter("@term", NpgsqlTypes.NpgsqlDbType.Text) { Value = DBNull.Value });

                using (var reader = await command.ExecuteReaderAsync().ConfigureAwait(false))
                {
                    while (await reader.ReadAsync().ConfigureAwait(false))
                    {
                        var item = new SearchPatients(
            ...
            return new Result<ImmutableList<SearchPatients>, SqlError>.Ok<ImmutableList<SearchPatients>, SqlError>(results.ToImmutable());
        }
        catch (Exception ex)
        {
            return new Result<ImmutableList<SearchPatients>, SqlError>.Error<ImmutableList<SearchPatients>, SqlError>(new SqlError("Database error", ex));
        }
    }
}

/// <summary>
/// Result row for 'SearchPatients' query.
/// </summary>
public record SearchPatients
{
    ...
    /// <summary>Initializes a new instance of SearchPatients.</summary>
