...
/// <summary>
/// Extension methods for 'GetAppointmentsByPractitioner'.
/// </summary>
public static partial class GetAppointmentsByPractitionerExtensions
{
    /// <summary>
    /// Executes 'GetAppointmentsByPractitioner.sql' and maps results.
    /// </summary>
    /// <param name="connection">Open NpgsqlConnection connection.</param>
    /// <param name="practitionerReference">Query parameter.</param>
    /// <returns>Result of records or SQL error.</returns>
    public static async Task<Result<ImmutableList<GetAppointmentsByPractitioner>, SqlError>> GetAppointmentsByPractitionerAsync(this NpgsqlConnection connection, object practitionerReference)
    {
        const string sql = @"SELECT fhir_Appointment.Id, fhir_Appointment.Status, fhir_Appointment.ServiceCategory, fhir_Appointment.ServiceType, fhir_Appointment.ReasonCode, fhir_Appointment.Priority, fhir_Appointment.Description, fhir_Appointment.StartTime, fhir_Appointment.EndTime, fhir_Appointment.MinutesDuration, fhir_Appointment.PatientReference, fhir_Appointment.PractitionerReference, fhir_Appointment.Created, fhir_Appointment.Comment FROM fhir_Appointment WHERE fhir_Appointment.PractitionerReference = @practitionerReference AND fhir_Appointment.Status = 'booked' ORDER BY fhir_Appointment.StartTime ";

        try
        {
            var results = ImmutableList.CreateBuilder<GetAppointmentsByPractitioner>();

            using (var command = new NpgsqlCommand(sql, connection))
            {
                if (practitionerReference is not null and not DBNull)
                    command.Parameters.AddWithValue("@practitionerReference", practitionerReference);
                else
                    command.Parameters.Add(new NpgsqlParameter("@practitionerReference", NpgsqlTypes.NpgsqlDbType.Text) { Value = DBNull.Value });

                using (var reader = await command.ExecuteReaderAsync().ConfigureAwait(false))
                {
                    while (await reader.ReadAsync().ConfigureAwait(false))
                    {
                        var item = new GetAppointmentsByPractitioner(
            ...
            return new Result<ImmutableList<GetAppointmentsByPractitioner>, SqlError>.Ok<ImmutableList<GetAppointmentsByPractitioner>, SqlError>(results.ToImmutable());
        }
        catch (Exception ex)
        {
            return new Result<ImmutableList<GetAppointmentsByPractitioner>, SqlError>.Error<ImmutableList<GetAppointmentsByPractitioner>, SqlError>(new SqlError("Database error", ex));
        }
    }
}

/// <summary>
/// Result row for 'GetAppointmentsByPractitioner' query.
/// </summary>
public record GetAppointmentsByPractitioner
{
    ...
    /// <summary>Initializes a new instance of GetAppointmentsByPractitioner.</summary>
    public GetAppointmentsByPractitioner(
        string Id,
        string Status,
