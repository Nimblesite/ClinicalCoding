...
public sealed class PasskeyAuthProvider : IChallengeAuthProvider
{
    ...
    )
    {
        ...
        if (isNew)
        {
            ...
            _ = await tx.Insertgk_userAsync(userId, request.DisplayName ?? request.Email ?? userId, request.Email ?? string.Empty, now, null, true, null).ConfigureAwait(false);
        ...
        _ = await tx2.Insertgk_challengeAsync(challengeId, userId, options.Challenge, "registration", now, expiry).ConfigureAwait(false);
    ...
    public async Task<Result<AuthBeginResult, AuthError>> BeginLoginAsync(IDbConnection conn)
    {
        ...
        await using var tx = await npgsql.BeginTransactionAsync().ConfigureAwait(false);
        _ = await tx.Insertgk_challengeAsync(challengeId, null, options.Challenge, "authentication", now, expiry).ConfigureAwait(false);
        await tx.CommitAsync().ConfigureAwait(false);
    ...
    )
    {
        ...
        var storedSignCount = (uint)(storedCred.sign_count ?? 0);
        ...
        updateCmd.CommandText = "UPDATE gk_credential SET sign_count = @c, last_used_at = @n WHERE id = @id";
        ...
        // Update last login
        using var loginCmd = npgsql.CreateCommand();
        loginCmd.CommandText = "UPDATE gk_user SET last_login_at = @n WHERE id = @id";
        loginCmd.Parameters.AddWithValue("@n", now);
        loginCmd.Parameters.AddWithValue("@id", (object?)storedCred.user_id ?? DBNull.Value);
        await loginCmd.ExecuteNonQueryAsync().ConfigureAwait(false);
